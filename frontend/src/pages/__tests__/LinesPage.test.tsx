...
describe("LinesPage", () => {
  ...
  it("allows user to edit a line", async () => {
    const user = userEvent.setup();

    // Create a team and line first
    ...
    // Change the name
    const nameInput = screen.getByLabelText(/line name/i);
    await user.clear(nameInput);
    await user.type(nameInput, "D-Line Elite");
    ...
    await waitFor(() => {
      expect(screen.getByText("D-Line Elite")).toBeInTheDocument();
    });
  });

  it("allows user to delete a line", async () => {
    const user = userEvent.setup();
    ...
    await fetch("http://localhost:8000/lines", {
      ...
      body: JSON.stringify({
        name: "Handler Line",
        description: "Handler specialists",
        team_id: team.id,
      }),
    });
    ...
    // Wait for line to appear
    await waitFor(() => {
      expect(screen.getByText("Handler Line")).toBeInTheDocument();
    });
    ...
    await waitFor(() => {
      expect(screen.queryByText("Handler Line")).not.toBeInTheDocument();
    });
